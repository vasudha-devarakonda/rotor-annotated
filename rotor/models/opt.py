    ...
    def forward(
        ...
        if self._use_flash_attention_2:
            ...
            if attention_mask is None:
                ...
                raise ValueError(
                    f"Attention mask length {attention_mask.shape[1]} "
                    f"!= {mask_seq_length}"
                )
...
class OPTDecoderLayerWrapper(nn.Module):
    def __init__(self, layer: OPTDecoderLayer, config):
        super().__init__()
        self.layer = layer
        self.config = config
...
class OutputHead(nn.Module):
    def __init__(self, config):
        ...
        if config.hidden_size != config.word_embed_proj_dim:
...
class OPTForCausalLM(nn.Sequential):
    def __init__(self, config):
        ...
        self.add_module("output_head", OutputHead(config))
        self.add_module("lm_head", LMHead(config))
