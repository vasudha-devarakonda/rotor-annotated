...
class BasicConv2d(nn.Sequential):
    ...
    def __init__(self, input_channels, output_channels, use_checkpoint=False, **kwargs):
        super().__init__()
        self.add_module('conv', nn.Conv2d(input_channels, output_channels, bias=False, **kwargs))
...
class Inception_Stem(nn.Module):
    ...
    def forward(self, x):
        ...
        x = [
            self.branch3x3_conv(x), #3
            self.branch3x3_pool(x) #1
        ]
        x = torch.cat(x, 1) #1

        x = [
            self.branch7x7a(x), #12
            self.branch7x7b(x) #6
        ]
        x = torch.cat(x, 1) #1

        x = [
            self.branchpoola(x), #1
            self.branchpoolb(x) #3
        ]
...
class InceptionA(nn.Module):
    ...
    def __init__(self, input_channels, use_checkpoint=False):
        super().__init__()

        self.branch3x3stack = nn.Sequential(
            BasicConv2d(input_channels, 64, kernel_size=1, use_checkpoint=use_checkpoint),
            BasicConv2d(64, 96, kernel_size=3, padding=1, use_checkpoint=False),
            BasicConv2d(96, 96, kernel_size=3, padding=1, use_checkpoint=use_checkpoint)
        )
        ...
        self.branch1x1 = BasicConv2d(input_channels, 96, kernel_size=1, use_checkpoint=use_checkpoint)

        self.branchpool = nn.Sequential(
            nn.AvgPool2d(kernel_size=3, stride=1, padding=1),
            BasicConv2d(input_channels, 96, kernel_size=1, use_checkpoint=use_checkpoint)
        )
...
class InceptionB(nn.Module):
    ...
    def __init__(self, input_channels, use_checkpoint=False):
        ...
        self.branch7x7stack = nn.Sequential(
            ...
            BasicConv2d(192, 192, kernel_size=(1, 7), padding=(0, 3), use_checkpoint=use_checkpoint),
            BasicConv2d(192, 224, kernel_size=(7, 1), padding=(3, 0), use_checkpoint=False),
            BasicConv2d(224, 224, kernel_size=(1, 7), padding=(0, 3), use_checkpoint=use_checkpoint),
            BasicConv2d(224, 256, kernel_size=(7, 1), padding=(3, 0), use_checkpoint=False),
        )

        self.branch7x7 = nn.Sequential(
            BasicConv2d(input_channels, 192, kernel_size=1),
            BasicConv2d(192, 224, kernel_size=(1, 7), padding=(0, 3), use_checkpoint=use_checkpoint),
            BasicConv2d(224, 256, kernel_size=(7, 1), padding=(3, 0), use_checkpoint=False),
        )

        self.branch1x1 = BasicConv2d(input_channels, 384, kernel_size=1, use_checkpoint=use_checkpoint)

        self.branchpool = nn.Sequential(
            nn.AvgPool2d(3, stride=1, padding=1),
            BasicConv2d(input_channels, 128, kernel_size=1, use_checkpoint=use_checkpoint)
        )
...
class ReductionB(nn.Module):
    ...
    def __init__(self, input_channels, use_checkpoint=False):
        ...
        self.branch7x7 = nn.Sequential(
            ...
            BasicConv2d(256, 256, kernel_size=(1, 7), padding=(0, 3), use_checkpoint=use_checkpoint),
            BasicConv2d(256, 320, kernel_size=(7, 1), padding=(3, 0), use_checkpoint=False),
            BasicConv2d(320, 320, kernel_size=3, stride=2, padding=1, use_checkpoint=use_checkpoint),
        )

        self.branch3x3 = nn.Sequential(
            BasicConv2d(input_channels, 192, kernel_size=1, use_checkpoint=use_checkpoint),
            BasicConv2d(192, 192, kernel_size=3, stride=2, padding=1, use_checkpoint=False)
        )
...
class InceptionC(nn.Module):
    ...
    def __init__(self, input_channels, use_checkpoint=False):
        ...
        self.branch3x3stack = nn.Sequential(
            ...
            BasicConv2d(384, 448, kernel_size=(1, 3), padding=(0, 1), use_checkpoint=use_checkpoint),
            BasicConv2d(448, 512, kernel_size=(3, 1), padding=(1, 0), use_checkpoint=False),
        )
        self.branch3x3stacka = BasicConv2d(512, 256, kernel_size=(1, 3), padding=(0, 1), use_checkpoint=use_checkpoint)
        self.branch3x3stackb = BasicConv2d(512, 256, kernel_size=(3, 1), padding=(1, 0), use_checkpoint=False)

        self.branch3x3 = BasicConv2d(input_channels, 384, kernel_size=1, use_checkpoint=use_checkpoint)
        self.branch3x3a = BasicConv2d(384, 256, kernel_size=(3, 1), padding=(1, 0), use_checkpoint=False)
        self.branch3x3b = BasicConv2d(384, 256, kernel_size=(1, 3), padding=(0, 1), use_checkpoint=use_checkpoint)

        self.branch1x1 = BasicConv2d(input_channels, 256, kernel_size=1)

        self.branchpool = nn.Sequential(
            nn.AvgPool2d(kernel_size=3, stride=1, padding=1),
            BasicConv2d(input_channels, 256, kernel_size=1, use_checkpoint=use_checkpoint)
        )
...
class InceptionV4(nn.Sequential):
    ...
    def __init__(
        ...
        # Stem
        self.add_module('Stem', Inception_Stem(3))


        self.add_module(
            'InceptionA',
            self._generate_inception_module(
                384, 384, A, InceptionA, use_checkpoint
            )
        )


        reduction_a = ReductionA(384, k, l, m, n)
        self.add_module('ReductionA', reduction_a)

 
        self.add_module(
            'InceptionB',
            self._generate_inception_module(
                reduction_a.output_channels, 1024, B, InceptionB, use_checkpoint
            )
        )


        self.add_module('ReductionB', ReductionB(1024))

        self.add_module(
            'InceptionC',
            self._generate_inception_module(
                1536, 1536, C, InceptionC, use_checkpoint
            )
        )


        self.add_module('AvgPool', nn.AvgPool2d(7))
        self.add_module('Dropout', nn.Dropout2d(1 - 0.8))
        self.add_module('Flatten', nn.Flatten())
        self.add_module('Linear', nn.Linear(1536, class_nums))

    @staticmethod
    def _generate_inception_module(
        input_channels,
        output_channels,
        block_num,
        block,
        ...
        for i in range(block_num):
            if i % 2 == 0:
                layers.add_module(
                    f"{block.__name__}_{i}",
                    block(input_channels)
                )
            else:
                layers.add_module(
                    f"{block.__name__}_{i}",
                    block(input_channels, use_checkpoint=use_checkpoint)
                )
