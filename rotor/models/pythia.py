...
#     """
#     GPTNeoX Model with a language modeling head on top for causal LM.
#     Combines GPTNeoXModel and GPTNeoXForCausalLM into a single class.
#     """
#     def __init__(self, config):
#         super().__init__()
#         self.config = config
...
#                 attention_mask = attention_mask[:, None, None, :]
#                 attention_mask = attention_mask.to(dtype=self.embed_in.weight.dtype)
#                 attention_mask = (1.0 - attention_mask) * torch.finfo(self.embed_in.weight.dtype).min
