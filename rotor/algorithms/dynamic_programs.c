...
#define WHAT(m, i, l) what[(m)*(chain_length+1)*(chain_length+1) + (i) * (chain_length+1) + (l)]
  ...
  for(long m = 0; m <= mmax; ++m)
	...
	if ((m >= mmin)) {
	  ...
	  /// sumFw + OPT(m-cw[i+1], i+1, l) + OPT(m, i, i); // Value for j = i+1
...
inline long floating_index_in_array(long m_factor, long m, long i, long j, long k) { 
  return m * m_factor + (i*(i+1)*(2*i+4))/12 + (i+1)*j - (j*(j-1))/2 + k;
}

typedef struct {
  long sp; long r; long tp;
} index_t; 

static PyObject *
floating_compute_table(PyObject *self, PyObject *args)
{
  PyObject* chain_param;
  int mmax; 
...
#define WHAT(m, s, t, l) what[floating_index_in_array(m_factor, (m), chain_length - (s), (t) - (s), (l) - (t))]
  ...
  double* partialSumsFW = calloc(chain_length + 1, sizeof(double));
  double total = 0; 
  for(long i = 0; i < chain_length; ++i) {
  ...
  for(long m = 0; m <= mmax; ++m)
	...
	for(long j = s+1; j < l; ++j) {
	  ...
	  if (val > memNullSecond)
	    memNullSecond = val; 
	}
	for(long t = s; t <= l; ++t) {
	  double chainCost = INFINITY; 
	  if ( (s == t)
	       && (m >= cw[l+1] + cbw[s+1] + fwd_tmp[s])
	  ...
	  if (m >= memNullFirst && m >= cw[l+1] + memNullSecond) {
		...
		for(long tp = t+1; tp <= l; ++tp)
		  for(long sp = r+1; sp <= tp; ++sp) {
			...
			+ OPT(mp-cw[sp], sp, tp, l) + OPT(mp, r, t, tp-1);
  ...
  for(long m = 0; m <= mmax; ++m) {
	...
	for(long l = t; l <= chain_length; ++l) {
	  ...
	  if (idx_what->sp >= 0) 
	    value_what = Py_BuildValue("(O(lll))", Py_False,
				       idx_what->sp, idx_what->r, idx_what->tp);
	  PyDict_SetItem(res_what_m, key, value_what);
	  if (value_what != true_tuple)
	    Py_DECREF(value_what); 
	  Py_DECREF(key);
	  Py_DECREF(value_opt);
	}
  }
...
static PyObject *
griewank_heterogeneous_compute_table(PyObject *self, PyObject *args)
{
  PyObject* chain_param;
  int mmax; 
...
#define OPT(m, i, l) opt[(m)*(chain_length+1)*(chain_length+1) + (i) * (chain_length+1) + (l)]
  ...
  double* sumfw = calloc(chain_length, sizeof(double));
  double* sumbw = calloc(chain_length+1, sizeof(double));
  double* sumsumfw = calloc(chain_length, sizeof(double));
  ...
  for(long m = 0; m <= mmax; ++m)
	...
	if ((m >= mmin)) {
	  ...
	  noCheckpointCost += sumsumfw[l-1] - (i > 0 ? sumsumfw[i-1] + (l-i)*sumfw[i-1] : 0); 
	  ...
	  if (m >= cw[i]) {
	    ...
	    for (long j = i+1; j < l; ++j) {
	      sumFwds += fw[j-1];
	      valueCost = fmin(valueCost, 
			       sumFwds + OPT(m-cw[i], j, l) + OPT(m, i, j-1));
	    }
	  }
...
static PyMethodDef dynamic_programs_methods[] = {
    ...
    {"griewank_heterogeneous_compute_table",  griewank_heterogeneous_compute_table, METH_VARARGS,
