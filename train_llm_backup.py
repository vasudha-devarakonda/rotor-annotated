...
def test_model_memory(model, input_ids, labels, device):
    ...
    # Max GPU memory
    print(f"{2*(m_a-m_b)/(1024**2)} we have")
    return loss.item(), 2*(mem_after - mem_before)
...
if __name__ == '__main__':
    ...
    for mod, ops in module_to_ops.items():
        ...
        print(f"{mod}: {ops}")
    print("============================")
    filename = f"recomp-llm-opt/{model_name}_{int(args.budget)}_{int(mem_loss/(1024**3))}_recompops.txt"
